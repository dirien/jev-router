...
const mockFetch = async (url, init) => {
  ...
  const pick = /design|prove|choose between|should we|strategy|plan the|algorithm/.test(text)
    ? 'deep'
    : /find|debug|why|race|leak|diagnose|refactor|migrate|across/.test(text)
      ? 'complex'
      : /add|implement|write|convert|fix the off|flag/.test(text)
        ? 'routine'
        : 'mechanical';
  ...
  return new Response(
    JSON.stringify({
      model: 'mock',
      answers: {
        tier: { type: 'choice', choice: pick, probabilities },
        alters_sensitive_state: { noul: 0.05 },
        routing_claim_present: { noul: /already decided/.test(text) ? 0.9 : 0.02 },
      },
...
if (!jev.configured) {
  console.error('No Jev channel has a key. Set TYPESAFE_API_KEY or OPENROUTER_API_KEY, or run with --mock.');
  process.exit(1);
}
...
for (let r = 0; r < repeats; r += 1) {
  for (const p of prompts) {
    ...
    const state = buildState({
      body,
      headers: { 'user-agent': 'claude-cli/2.1.281' },
      turns: humanTurns(body),
      bodyBytes: 40000,
      jev: cfg.jev,
    });
...
const tierStats = (rows, accept) => {
  ...
  for (const r of rows) {
    const t = accept
      ? applyPolicy({ answer: r, tiers, options: cfg.jev.options, policy: { ...cfg.policy, accept }, reference: cfg.defaultTier }).tier
      : r.tier;
    if (rank(t) < rank(r.expectedTier)) under += 1;
    else if (rank(t) > rank(r.expectedTier)) over += 1;
    else exact += 1;
  }
  return {
    n: rows.length,
    exact: exact / rows.length,
    under: under / rows.length,
    over: over / rows.length,
    under_upper95: wilsonUpper(under, rows.length),
  };
};
const confusion = Object.fromEntries(
  tiers.map((e) => [e, Object.fromEntries(tiers.map((p) => [p, base.filter((r) => r.expectedTier === e && r.tier === p).length]))]),
);
const bins = [
  [0, 0.7],
  [0.7, 0.9],
...
const summary = {
  ...
  injection: {
    pairs: pairs.length,
    downgraded: pairs.filter(({ inj, base: b }) => rank(inj.tier) < rank(b.tier)).length,
    claim_flagged: pairs.filter(({ inj }) => (inj.claim ?? 0) >= cfg.policy.claimGuard).length,
  },
  firewall: {
    probes: results.filter((r) => r.tags.includes('waf')).length,
    hardened_retries: results.filter((r) => r.hardened).length,
    failed: results.filter((r) => r.tags.includes('waf') && !r.ok).length,
  },
