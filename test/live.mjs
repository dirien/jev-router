...
import { claudeCodeBody, claudeCodeHeaders, claudeCodeToolTurn, jevOptionsAnswer, json, listen, mockServer, sha256, close } from './helpers.mjs';
...
const logs = [];
const server = createRouter(validateConfig({ ...cfg, stateFile: null }), { env: { ...process.env, MOCK_JEV_KEY: 'mock' }, log: (e) => logs.push(e) });
const url = await listen(server);
const headers = (s) => claudeCodeHeaders(s, { 'x-claude-code-request-class': 'main', 'anthropic-beta': 'claude-code-20250219,interleaved-thinking-2025-05-14,context-management-2025-06-27,effort-2025-11-24' });
...
async function send(label, body, hdrs) {
  const h = { ...hdrs }; delete h['x-api-key'];
  ...
  results.push({ label, status: res.status, tier: res.headers.get('x-jev-tier'), model: route?.model, reason: route?.reason, sha_match: done?.sha256 === sha256(bytes), usage: done?.usage, cost_usd: done?.cost_usd, error: res.status >= 400 ? bytes.toString().slice(0, 160) : undefined });
}
const s1 = crypto.randomUUID();
await send('1 haiku, full Claude Code field set', opus(s1, 'What does ls -la print? One sentence.'), headers(s1));
const loop = { ...claudeCodeToolTurn(s1, 'What does ls -la print? One sentence.'), model: 'claude-opus-5-5', max_tokens: 32, thinking: { type: 'adaptive' }, output_config: { effort: 'high' }, context_management: { edits: [{ type: 'clear_thinking_20251015', keep: 'all' }] } };
...
await close(server); await jev.close();
