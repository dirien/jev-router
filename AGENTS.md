...
| Keys | The environment, or an env file of `KEY=VALUE` lines: `--env-file`, else `JEV_ROUTER_ENV_FILE`, else `$XDG_CONFIG_HOME/jev-router/env` when it exists (source "default location"; `setup` writes it, mode 0600), loaded with `process.loadEnvFile` before anything reads the environment (`loadEnvFile` in `src/envfile.mjs`). Variables already set win; `launch` keeps the file's variables away from the agent |
| Setup | `jev-router setup` (`runSetup` in `src/setup.mjs`): asks everything first (`Prompter` in `src/prompt.mjs`), checks the Jev key with one `JevClient.decide`, then writes the config, the env file (`saveEnvValues`), the service (`src/service.mjs`, after a global `npm install -g` when run from npx: `src/install.mjs`) and, once the router answers, Claude Code's `settings.json` with a backup. `$XDG_CONFIG_HOME/jev-router/setup.json` records its changes without secrets; `jev-router uninstall` (`runUninstall`) reads it |
...
| launchd `bootout`/`bootstrap gui/<uid>`, and `systemctl --user show-environment`/`daemon-reload`/`enable`/`restart`/`disable --now` | man pages; **not run** against a real launchd or systemd user manager here: `test/fakes/fake-service.mjs` stands in | `startService` and `stopService` in `src/service.mjs`; bootstrap is retried for launchd, which may still be unloading the old agent |
...
| Module | May import |
| --- | --- |
| `config`, `files`, `logfile`, `messages`, `net`, `prompt`, `secrets`, `sessions`, `ui`, `usage` | Node built-ins only, nothing from `src/` |
| `envfile`, `install`, `service` | `files` |
| `claude` | `files`, `net` |
