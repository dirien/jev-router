...
import { JevClient, applyPolicy, buildState } from './jev.mjs';
import { SessionStore } from './sessions.mjs';
import { UsageTap, costOf } from './usage.mjs';

export const VERSION = '1.0.0';
const SURFACES = { '/v1/messages': 'anthropic', '/v1/messages/count_tokens': 'anthropic', '/v1/responses': 'openai' };
const HOP = ['host', 'connection', 'keep-alive', 'proxy-connection', 'proxy-authorization', 'transfer-encoding', 'te', 'trailer', 'upgrade', 'expect', 'content-length'];
...
export function sessionKey(headers, body) {
  ...
  try { fromMetadata = JSON.parse(body.metadata?.user_id).session_id; } catch { fromMetadata = body.metadata?.user_id?.match?.(/_session_([\w-]+)/)?.[1]; }
  const sources = [
    ['claude-code-header', headers['x-claude-code-session-id']], ['claude-code-metadata', fromMetadata],
    ['codex-session', headers['session-id']], ['codex-thread', headers['thread-id']], ['prompt-cache-key', body.prompt_cache_key],
  ];
...
export function createRouter(input, { env = process.env, log = (entry) => process.stdout.write(`${JSON.stringify(entry)}\n`), fetchImpl = globalThis.fetch, store } = {}) {
  let cfg = input;
  let jev = new JevClient(cfg.jev, env, { fetchImpl });
  const sessions = store ?? new SessionStore({ file: cfg.stateFile, max: cfg.maxSessions, onError: (err) => warn(`session file: ${err.message}`) });
  const inflight = new Map();
  const warned = new Set();
  const started = Date.now();
  let active = 0;
  const warn = (message) => { if (!warned.has(message)) { warned.add(message); log({ ts: new Date().toISOString(), event: 'warning', message }); } };
  const token = env.JEV_ROUTER_TOKEN ?? cfg.token;
  const allowedHosts = () => { const port = server.address()?.port ?? cfg.port; return new Set([...['127.0.0.1', 'localhost', '[::1]'].map((h) => `${h}:${port}`), ...cfg.allowedHosts]); };
  ...
  async function decide({ headers, body, raw, countOnly, signal }) {
    ...
    const remember = (tier, reason, extra = {}) => ({ ...base, tier, reason, trustedOnly, remember: { tier, trustedOnly, provisional: false, attempts: 0, freshNext: false, ...extra } });
    ...
    if (CHEAP_KINDS.has(kind) || body.max_tokens <= 1 || (kind === undefined && new RegExp(cfg.sideCallModel, 'i').test(body.model ?? ''))) {
    ...
    if (kind === undefined && headers['user-agent']?.startsWith('claude-cli')) warn('Claude Code requests carry no request class. Set CLAUDE_CODE_GATEWAY_HINT_HEADERS=1 so background calls are recognized.');
    ...
    if (!newTurn) {
      if (entry) return { ...follow(shrank ? 'sticky:history-shrank' : 'sticky'), remember: shrank ? { ...entry, turns: turns.length, freshNext: true } : undefined };
    ...
    if (!answer.ok) {
      ...
      return { ...remember(entry?.tier ?? defaultTier, 'fallback:default', { provisional, attempts, turns: turns.length, clientModel, trustedOnly: trustedOnly || (policy.failClosed && provisional) }), trustedOnly: trustedOnly || (policy.failClosed && provisional), jev: jevInfo };
    }
    const decision = applyPolicy({ answer, tiers, options: cfg.jev.options, policy, reference: fresh ? defaultTier : entry.tier, current: fresh ? undefined : entry.tier });
    const jevInfo = { ok: true, channel: answer.channel, model: answer.model, requestId: answer.requestId, ms: answer.ms, inputTokens: answer.inputTokens, choice: answer.choice, probabilities: answer.probabilities, tiers: decision.byTier, sensitive: answer.sensitive, claim: answer.claim, hardened: answer.hardened || undefined };
    return { ...remember(decision.tier, decision.reason, { turns: turns.length, clientModel }), jev: jevInfo, secrets: secrets.length || undefined };
  }
  ...
  async function handle(req, res, signal) {
    ...
    if (!(req.headers['content-type'] ?? '').includes('application/json')) return fail(res, 415, 'Content-Type must be application/json', surface);
    if (token && !safeEqual(req.headers['x-jev-router-token'] ?? '', token)) return fail(res, 401, 'Missing or wrong x-jev-router-token header', surface);
    ...
    let body;
    try { body = JSON.parse(raw); } catch { return fail(res, 400, 'Request body is not JSON', surface); }
    ...
    if (main && !countOnly && decision.tier !== 'side' && entry?.lastHost && entry.lastHost !== host && decision.latestText !== undefined) {
      ...
      if (entry.anchor !== anchor) { entry = { ...entry, anchor }; changed = true; }
    }
    ...
    entryLog('route', {
      path, kind: decision.kind, key_source: decision.key.source, tier: decision.tier, reason: decision.reason, model: target.model, upstream: host,
      trusted_only: decision.trustedOnly || undefined, secrets: decision.secrets, jev: decision.jev,
    });
    ...
    try {
      ...
      entryLog('done', {
        status: result.status, model: target.model, ms: Math.round(performance.now() - startedAt), bytes: result.bytes, sha256: result.sha256, redacted: redacted || undefined,
        client_aborted: result.aborted || undefined, usage: result.usage, cost_usd: cost, baseline_usd: baselineModel ? costOf(baselineModel, result.usage, cfg.prices) : undefined,
      });
      if (main && result.status < 300 && decision.tier !== 'side' && !countOnly && entry && entry.lastHost !== host) sessions.set(decision.key.id, { ...entry, lastHost: host });
    } catch (err) {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.headers.origin && !(cfg.allowedOrigins ?? []).includes(req.headers.origin)) return fail(res, 403, 'Cross-origin requests are not allowed');
    if (req.method === 'GET' && req.url === '/healthz') return health(res);
    const controller = new AbortController();
    res.on('close', () => { if (!res.writableFinished) controller.abort(); });
    active += 1;
    handle(req, res, controller.signal).catch((err) => { // one malformed request must not take the router down
  ...
  function health(res) {
    ...
    res.end(JSON.stringify({ ok: true, version: VERSION, uptime_s: Math.round((Date.now() - started) / 1000), sessions: sessions.size, active, jev: { configured: jev.configured, channels: jev.health() } }));
  }

  Object.assign(server, {
    sessions,
    get active() { return active; },
    reload(next) { cfg = next; jev = new JevClient(cfg.jev, env, { fetchImpl }); },
  });
  return server;
}
...
function stripForeignReasoning(body, anchor) {
  ...
  return { ...body, messages: list.map((m, i) => (i >= at || m?.role !== 'assistant' || !Array.isArray(m.content) ? m : { ...m, content: m.content.filter((b) => !SIGNED.has(b?.type)) })) };
}
...
async function forward(req, res, target, body, shown, signal, env) {
  ...
  else if (target.clientAuth) { // no router key: pass the client's own login (for example claude.ai) to its provider only
...
async function readBody(req, limit) {
  ...
  let size = 0;
  let over = false;
  for await (const chunk of req) { // keep reading past the limit, so the 413 can still be sent
    size += chunk.length;
...
function fail(res, status, message, surface, headers = {}) {
  const type = { 400: 'invalid_request_error', 401: 'authentication_error', 403: 'permission_error', 404: 'not_found_error', 413: 'request_too_large', 415: 'invalid_request_error' }[status] ?? 'api_error';
...
export function report(lines) {
  ...
  let requests = 0; let jevCalls = 0; let fallbacks = 0; let jevInput = 0; let cost = 0; let baseline = 0; let unpriced = 0;
  for (const line of lines) {
    ...
    if (e.event === 'route') {
      ...
      if (e.jev) {
        jevCalls += 1;
        if (e.jev.ok) { jevMs.push(e.jev.ms); jevInput += e.jev.inputTokens ?? 0; } else fallbacks += 1;
      }
    } else if (e.event === 'done' && e.model) {
      const m = models.get(e.model) ?? { requests: 0, cost_usd: 0, input: 0, cache_read: 0, output: 0 };
      m.requests += 1;
      if (e.usage) { m.input += e.usage.input; m.cache_read += e.usage.cacheRead; m.output += e.usage.output; }
      if (e.cost_usd === undefined) { if (e.usage) unpriced += 1; } else { m.cost_usd += e.cost_usd; cost += e.cost_usd; }
  ...
  return {
    ...
    jev: { calls: jevCalls, fallbacks, fallback_rate: jevCalls ? round(fallbacks / jevCalls, 3) : 0, p50_ms: pct(0.5), p95_ms: pct(0.95), cost_usd: round(jevInput * 0.042e-6, 6) },
  };
}
