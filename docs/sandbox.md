...
sbx create --name jev-router --kit ghcr.io/dirien/jev-router-kit:1.6.0 claude "$WS"   # flags verified; kit not run yet
```

Leave out the `printf … |` part to type a key at a prompt instead.

The release workflow publishes the kit to GHCR for every release: `:1.6.0` pins this release, and `:latest` follows
the newest one. The package is public, so pulling it needs no login. sbx can also read the kit from git:

```bash
sbx create --name jev-router --kit "git+https://github.com/dirien/jev-router.git#ref=v1.6.0&dir=sbx/jev-router-kit" claude "$WS"
```
...
| `reason: no-jev` | No channel has a key in the router's environment, so the kit isn't attached. Run `sbx kit add jev-router ghcr.io/dirien/jev-router-kit:1.6.0` on the host; this recreates the container. |
